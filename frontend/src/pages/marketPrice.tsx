...
// //       style={{ backgroundImage: "url('/assets/background8.jpeg')" }} // Replace with your image path
// //     >
// //       <div className="absolute inset-0  bg-opacity-40 backdrop-blur-sm"></div>

// //       <div className="relative z-10 max-w-3xl w-full p-8 bg-white/20 backdrop-blur-xl rounded-xl shadow-lg  mt-15">
...
// //           <div>
// //             <label className="block text-black text-2xl font-bold">State</label>
// //             <input
// //               type="text"
// //               value={state}
...
// //           </div>
// //           <div>
// //             <label className="block text-black text-2xl font-bold">District (Optional)</label>
// //             <input
// //               type="text"
// //               value={district}
...
// //           </div>
// //           <div>
// //             <label className="block text-black text-2xl font-bold">Crop Name (ALL CAPS)</label>
// //             <input
// //               type="text"
// //               value={cropName}
...
//       if (result.crop_prices) setPriceData(result.crop_prices);
//       if (result.price_trends) setPriceTrends(result.price_trends);
//       if (!result.crop_prices && !result.price_trends) setError("No data found.");
//     } catch (err) {
//       console.error("Error fetching market prices:", err);
//       setError("Failed to fetch market prices.");
//     } finally {
...
//   return (
//     <div className="relative min-h-screen flex items-center justify-center bg-cover bg-center" style={{ backgroundImage: "url('/assets/background8.jpeg')" }}>
//       <div className="absolute inset-0 bg-opacity-40 backdrop-blur-sm"></div>
//       <div className="relative z-10 max-w-3xl w-full p-8 bg-white/20 backdrop-blur-xl rounded-xl shadow-lg">
//         <motion.h2 className="text-3xl font-bold text-black text-center mb-6" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
//           📊 Market Price Analysis
//         </motion.h2>
        
//         <form onSubmit={handleSubmit} className="space-y-5">
//           {[['State', state, setState, true], ['District (Optional)', district, setDistrict, false], ['Crop Name (ALL CAPS)', cropName, setCropName, true]].map(([label, value, setter, required], idx) => (
//             <div key={idx}>
//               <label className="block text-black text-2xl font-bold">{label}</label>
//               <input
//                 type="text"
//                 value={value}
...
//           {warning && <p className="text-yellow-500 text-sm text-center">⚠ {warning}</p>}
//           {error && <p className="text-red-600 text-center font-medium bg-red-100 p-3 rounded-lg">❌ {error}</p>}
          
//           <button type="submit" className="w-full bg-gradient-to-r from-green-400 to-blue-500 text-white py-3 text-lg rounded-lg font-semibold hover:scale-105 transition duration-300 shadow-lg" disabled={loading}>
//             {loading ? "Loading..." : "Get Market Price"}
//           </button>
//         </form>
...
//           <div className="mt-6">
//             <h3 className="text-lg font-semibold text-center mb-3">💰 Market Prices</h3>
//             <div className="overflow-y-auto max-h-60 border border-gray-200 rounded-lg p-3 bg-white/10">
...
//         {priceTrends && !loading && (
//           <div className="mt-8 p-4 bg-white/10 rounded-lg shadow">
//             <h3 className="text-lg font-semibold text-center text-white mb-3">📊 Price Trends Over Time</h3>
//             <div className="w-full h-72">
//               <Line data={{
//                 labels: priceTrends.map(entry => entry.Date),
//                 datasets: [
//                   { label: "Max Price", data: priceTrends.map(entry => entry.MaxPrice), borderColor: "rgb(75, 192, 192)", borderWidth: 2, tension: 0.3, fill: false },
//                   { label: "Min Price", data: priceTrends.map(entry => entry.MinPrice), borderColor: "rgb(255, 99, 132)", borderWidth: 2, tension: 0.3, fill: false },
//                 ],
//               }} options={{ responsive: true, maintainAspectRatio: false }} />
//             </div>
//           </div>
